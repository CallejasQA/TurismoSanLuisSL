<?php cabecera('Mi agenda'); ?>
  ...
  <section class="agenda-wrapper">
    <header class="agenda-header">
      <h1>Mi agenda de ocupación</h1>
      <p>Visualiza tus reservas por mes, alojamiento o día específico.</p>
    </header>
    ...
    <form class="agenda-filters" method="get" action="index.php" id="formFiltrosAgenda">
      ...
      <div class="filter-group">
        ...
        <input id="mes" type="month" name="mes" value="<?= htmlspecialchars($mesSeleccionado ?? '') ?>">
      </div>
      <div class="filter-group">
        <label for="dia">Día específico</label>
        <input id="dia" type="date" name="dia" value="<?= htmlspecialchars($diaSeleccionado ?? '') ?>" placeholder="AAAA-MM-DD">
      </div>
      <div class="filter-actions">
        <button type="submit" class="btn-primary">Aplicar filtros</button>
        <button type="button" class="btn-secondary" id="btnLimpiarFiltros">Limpiar</button>
      </div>
    </form>
  ...
  <style>
    ...
    .filter-group input[type="date"] { padding: 8px 10px; border-radius: 8px; border: 1px solid #d1d5db; font-size: 0.85rem; }
    .filter-actions { display: flex; gap: 8px; align-items: center; }
    .btn-primary { background: #2563eb; color: #fff; border: none; padding: 9px 16px; border-radius: 10px; font-weight: 600; cursor: pointer; }
